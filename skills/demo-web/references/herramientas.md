...
| Casilla de Lume por IMAP | Deja el mail de la demo como borrador (`crear-borrador.mjs`) | La casilla @lumeai.uy está en el hosting propio (Dovecot en `mail.lumeai.uy`), no en Microsoft ni Google: los conectores de Outlook/Gmail no la alcanzan. Guardar la contraseña como `LUME_IMAP_PASS` (`setx` desde un PowerShell aparte); opcionales `LUME_IMAP_USER`, `LUME_IMAP_HOST`, `LUME_IMAP_PORT`. |
